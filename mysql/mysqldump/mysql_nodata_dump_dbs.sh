...
# Root dir
MYHOME=`echo $HOME`
DUMPDIRROOT="${MYHOME}/mysql/mysqlbackup"
# Main logical dir
...
for DB in `echo ${DBLISTQRY} |mysql --login-path=prod -N`; do
	...
	#mysqldump --login-path=prod $ONLYSCHEMADUMPPARAMS ${DB} | gzip > "${DBBACKUPDIR}/${DB}.schema.sql"
	mysqldump --login-path=prod $ONLYSCHEMADUMPPARAMS ${DB} > "${DBBACKUPDIR}/${DB}.schema.sql"
	if [[ $? -ne 0 ]]; then
	...
	ROUTCOUNT=`echo ${DBROUTINESQRY} | mysql --login-path=prod -N`
	TRIGCOUNT=`echo ${DBTRIGGERSQRY} | mysql --login-path=prod -N`
	...
	if [[ $RTACCOUNT -eq 0 ]]; then
		...
		#mysqldump --login-path=prod $ONLYROUTINESDUMPPARAMS ${DB} | gzip > "${DBBACKUPDIR}/${DB}.routines.sql"
		mysqldump --login-path=prod $ONLYROUTINESDUMPPARAMS ${DB}  > "${DBBACKUPDIR}/${DB}.routines.sql"
		if [[ $? -ne 0 ]]; then
