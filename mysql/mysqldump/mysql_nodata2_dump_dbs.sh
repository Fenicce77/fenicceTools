...
DUMPDIRROOT=`grep "myrootdumpdir" ${MYCONFILE} |  awk -F'=' '{print $2}'`
# Backup directories
# Root dir
...
ONLYSCHEMADUMPPARAMS="--no-data --skip-triggers --single-transaction --column-statistics=0"
#ONLYDATADUMPPARAMS="--no-create-info --skip-triggers --single-transaction --column-statistics=0"
ONLYROUTINESDUMPPARAMS="--routines --no-create-info --no-data --single-transaction --column-statistics=0"
...
# execute above query 
for DB in `echo ${DBLISTQRY} |mysql -u${MYBKPUSR} -h${MYSRV} -p${MYBKPPWD} -N`; do
	...
	echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][INFO][DB=${DB}] Dumping Schema Definition for ${DB} into ${DBBACKUPDIR}/${DB}.schema.sql" >> ${BACKUPLOGFILE}
	echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][INFO][DB=${DB}] Dumping Schema Definition for ${DB} into ${DBBACKUPDIR}/${DB}.schema.sql"
	mysqldump $ONLYSCHEMADUMPPARAMS --host=${MYSRV} -p${MYBKPPWD} ${DB} > "${DBBACKUPDIR}/${DB}.schema.sql"
	if [[ $? -ne 0 ]]; then
		echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][ERROR][DB=${DB}] Dumping Schema Definition into ${DBBACKUPDIR}/${DB}.schema.sql FAILED" >> ${BACKUPLOGFILE}
		echo "${yel}[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][ERROR][DB=${DB}] Dumping Schema Definition into ${DBBACKUPDIR}/${DB}.schema.sql FAILED${off}"
		ERRORCODE=20
	else
		echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][OK][DB=${DB}] Dumping Schema Definition into ${DBBACKUPDIR}/${DB}.schema.sql SUCCEED!!" >> ${BACKUPLOGFILE}
		echo "${grn}[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][OK][DB=${DB}] Dumping Schema Definition into ${DBBACKUPDIR}/${DB}.schema.sql SUCCEED!!${off}"
	fi
	# data dump 
	...
	# mysqldump $ONLYDATADUMPPARAMS ${DB}  > "${DBBACKUPDIR}/${DB}.data.sql.gz"
	# if [[ $? -ne 0 ]]; then
	...
	ROUTCOUNT=`echo ${DBROUTINESQRY} | mysql -u${MYBKPUSR} -h${MYSRV} -p${MYBKPPWD} -N`
	TRIGCOUNT=`echo ${DBTRIGGERSQRY} | mysql -u${MYBKPUSR} -h${MYSRV} -p${MYBKPPWD} -N`
	...
	if [[ $RTACCOUNT -eq 0 ]]; then
		...
		echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][INFO][DB=${DB}] Dumping Schema Routines for ${DB} into ${DBBACKUPDIR}/${DB}.routines.sql" >> ${BACKUPLOGFILE}
		echo "${yel}[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][INFO][DB=${DB}] Dumping Schema Routines for ${DB} into ${DBBACKUPDIR}/${DB}.routines.sql${off}"
		mysqldump $ONLYROUTINESDUMPPARAMS --host=${MYSRV} -p${MYBKPPWD} ${DB} > "${DBBACKUPDIR}/${DB}.routines.sql"
		if [[ $? -ne 0 ]]; then
			echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][ERROR][DB=${DB}] Dumping Routines Definition into ${DBBACKUPDIR}/${DB}.schema.sql FAILED" >> ${BACKUPLOGFILE}
			echo "${yel}[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][ERROR][DB=${DB}] Dumping Routines Definition into ${DBBACKUPDIR}/${DB}.schema.sql FAILED!! ${off}"
			ERRORCODE=22
		else
			echo "[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][OK][DB=${DB}] Dumping Routines Definition into ${DBBACKUPDIR}/${DB}.schema.sql SUCCEED!!" >> ${BACKUPLOGFILE}
			echo "${grn}[`date +"%Y-%m-%d %H:%M:%S"`][BACKUP][FULL][OK][DB=${DB}] Dumping Routines Definition into ${DBBACKUPDIR}/${DB}.schema.sql SUCCEED!! ${off}"
