...
mysqlbinlog --login-path=${LOGINPATH} --read-from-remote-server --base64-output=decode-rows -vv ${BINLOG_FILE} | awk 'BEGIN {s_type=""; s_count=0;count=0;CREATE_count=0;ALTER_count=0;delete_count=0;flag=0;} {if(match($0, /#*Table_map:.*mapped to number/)) {printf "Timestamp : " $1 " " $2 " Table : " $(NF-4); flag=1} else if (match($0, /(### CREATE TABLE .*..*)/)) {count=count+1;CREATE_count=CREATE_count+1;s_type="INSERT"; s_count=s_count+1;} else if (match($0, /(### ALTER TABLE .*..*)/)) {count=count+1;ALTER_count=ALTER_count+1;s_type="ALTER TABLE"; s_count=s_count+1;} else if (match($0, /(### DROP TABLE .*..*)/)) {count=count+1;delete_count=delete_count+1;s_type="DELETE"; s_count=s_count+1;} else if (match($0, /^(# at) /) && flag==1 && s_count>0) {print " Query Type : "s_type " " s_count " row(s) affected" ;s_type=""; s_count=0; } else if (match($0, /^(COMMIT)/)) {print "[Transaction total : " count " Insert(s) : " CREATE_count " ALTER TABLE(s) : " ALTER_count " Delete(s) : " delete_count "] \n+----------------------+----------------------+----------------------+----------------------+"; count=0;CREATE_count=0;ALTER_count=0; delete_count=0;s_type=""; s_count=0; flag=0} } '

# Usefull command execution :
# 1.- Check highest insert/ALTER TABLE/delete operation tables
