...
while true; do
    ...
    echo -e "  [k] Kill thread  |  [r] Refresh now  |  [q] Quit"
    echo -n "Press key: "
    ...
    case "$key" in
        ...
        k|K)
            ...
            tput cvvis
            echo -e "\n"
            read -p "Introduce el CONN_ID del hilo a matar: " KILL_ID
            if [[ "$KILL_ID" =~ ^[0-9]+$ ]]; then
                read -p "¿Seguro que deseas liquidar el hilo $KILL_ID? [y/N]: " CONFIRM
                if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
                    KILL_RES=$($MYSQLBIN --login-path="${LOGIN_PATH}" -e "KILL $KILL_ID;" 2>&1)
                    if [[ $? -eq 0 ]]; then
                        echo -e "${grn}[OK] Hilo $KILL_ID fulminado con éxito.${off}"
                    else
                        echo -e "${red}[ERROR] No se pudo matar el hilo $KILL_ID: $KILL_RES${off}"
                    fi
                else
                    echo -e "${yel}Operación de KILL cancelada.${off}"
                fi
            else
                if [[ -n "$KILL_ID" ]]; then echo -e "${red}[ERROR] El ID de conexión debe ser numérico.${off}"; fi
            fi
            sleep 2
            tput civis
            ;;
