...
assert_contains "$TMP/filter.sql" "p.USER IN ('app','reporting')" 'user filter is not an exact IN predicate'
assert_contains "$TMP/filter.sql" "p.DB IN ('sales')" 'database filter is not an exact IN predicate'
assert_contains "$TMP/filter.sql" "p.HOST IN ('host1:3306')" 'host filter is not an exact IN predicate'
...
assert_contains "$TMP/escaped-filter.sql" "p.USER IN ('o''reilly','back\\\\slash')" 'filter literals were not SQL-escaped'
...
new_sql_log "$TMP/own-kill.sql"
printf 'k999\nq' | "$SCRIPT" --login-path x --view transactions --mysql-bin "$FAKE" --no-color >"$TMP/own-kill.out"
assert_contains "$TMP/own-kill.out" 'Refusing to kill the monitor connection.' 'self-connection rejection missing'
assert_not_contains "$TMP/own-kill.sql" 'KILL CONNECTION' 'monitor connection was killed'
...
assert_contains "$TMP/interactive-filter.sql" "p.USER IN ('app','reporting')" 'interactive user filter was not applied'
assert_contains "$TMP/interactive-filter.sql" "p.DB IN ('sales')" 'interactive database filter was not applied'
assert_contains "$TMP/interactive-filter.sql" "p.HOST IN ('host1:3306')" 'interactive host filter was not applied'
