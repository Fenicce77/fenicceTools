...
SHOWSLAVEQRYOLD="SHOW REPLICA STATUS\G"
...
SKIPCOUNTQRYOLD="CALL mysql.rds_skip_repl_error;"
...
while [[ -f $LOCKFILE ]]; do
        SECSBEHIND=$(echo "SHOW REPLICA STATUS\G" | mysql --defaults-file=${MYCONF}| grep Seconds_Behind_Master | tr -d '[[:space:]]' | awk -F':' '{print $2}')
        #statements
        while [ $SECSBEHIND == 'NULL' ]; do
                #statements
                LASTSQLERRNO=$(echo "SHOW REPLICA STATUS\G" | mysql --defaults-file=${MYCONF}| grep "Last_SQL_Errno:" | awk -F':' '{print $2}' | tr -d '[[:space:]]')
                LASTSQLERR=$(echo "SHOW REPLICA STATUS\G" | mysql --defaults-file=${MYCONF}| grep "Last_SQL_Error:" | sed 's/Last_SQL_Error://g' | sed 's/^[[:space:]]*//' )
                case "$LASTSQLERRNO" in
                        ...
                        *)
                         ...
                         SECSBEHIND=$(echo "SHOW REPLICA STATUS\G" | mysql --defaults-file=${MYCONF} | grep Seconds_Behind_Master | tr -d '[[:space:]]' | awk -F':' '{print $2}')
                         echo "SHOW REPLICA STATUS\G" | mysql | grep Err | sed 's/^[[:space:]]*//'
