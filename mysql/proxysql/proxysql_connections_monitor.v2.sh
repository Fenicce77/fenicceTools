...
blk=$(tput blink)
bld=$(tput bold)
red=${bld}$(tput setaf 1)    # Red (Alerts / Increases / Errors)
grn=${bld}$(tput setaf 2)    # Green (Decreases / Success / Online)
yel=${bld}$(tput setaf 3)    # Yellow (Warnings / Thresholds)
blu=${bld}$(tput setaf 4)    # Blue (Info / Totals)
mag=${bld}$(tput setaf 5)    # Magenta (Highlights / Keys)
cyn=${bld}$(tput setaf 6)    # Cyan (Borders / Headers)
...
USER_FILTER=""
OUTPUT_FILE=""
VIEW_MODE="CONN"     # "CONN", "QUERY", or "BACKEND"
SORT_MODE="CONN"     # "CONN" (Desc) or "USER" (Asc)
THRESHOLD=0          # 0 means disabled
PAUSED=false         # Freeze screen state
...
usage() {
    ...
    echo -e "  ${grn}-r, --refresh-time=N${off}   Refresh time in seconds (Default: ${bld}5${off})."
    ...
    echo -e "  ${wht}1. Basic monitoring every 5 seconds:${off}"
    echo -e "     $0 ${mag}--login-path=proxysql_admin${off}"
    echo ""
    echo -e "  ${wht}2. Fast refresh with an alert if any user exceeds 50 connections:${off}"
    echo -e "     $0 ${mag}--login-path=proxysql_admin${off} ${grn}-r 2 -t 50${off}"
    echo -e "${cyn}=================================================================================${off}\n"
    exit 1
}
...
}
{
    if (NF > 0) {
        ...
        key = $1"\t"$2"\t"$3"\t"$4
        curr_val = $5
        prev_val = prev[key] ? prev[key] : curr_val
...
while true; do
    ...
    if ! $PAUSED; then
        if [[ "$VIEW_MODE" == "CONN" ]]; then
            ...
            POOL_DATA=$($MYSQL_CMD -e "$Q_POOL" 2>/dev/null)
            PING_DATA=$($MYSQL_CMD -e "$Q_PING" 2>/dev/null)
    ...
    PAUSE_LBL=$($PAUSED && echo "${bld}${blk}${red} [PAUSED] ${off}" || echo "")
    ...
    # ==============================================================================
    # Runtime Interactivity & Wait
    # ==============================================================================
    echo -e "\n${bld}Interactive Options:${off}"
    echo -e " [${mag}v${off}] ${blu}Toggle View${off} (Conn/Queries/Backend) | [${mag}r${off}] ${grn}Refresh${off} | [${mag}s${off}] ${grn}Sort${off} | [${mag}p${off}] ${yel}Pause/Resume${off} | [${mag}u${off}] ${grn}User Filter${off} | [${mag}t${off}] ${red}Threshold${off} | [${mag}q${off}] ${red}Quit${off}"
    ...
    case "$key" in
        ...
        p|P)
            ...
            ;;
        u|U) 
            echo -e "\n"; read -p "Enter new user filter (empty to disable): " USER_FILTER ;;
        t|T)
            echo -e "\n"; read -p "Enter connection threshold (0 to disable): " new_th
