...
NEWVER="8.0.22"
MYSQLVER=`echo $CHECKVERSIONQRY | mysql --login-path=${LOGINPATH} -N`
# MySQL version check
...
SECSBEHIND=$(echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep Seconds_Behind_Master | tr -d '[[:space:]]' | awk -F':' '{print $2}')
while [ $SECSBEHIND == 'NULL' ]; do
        #statements
        LASTSQLERRNO=$(echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep "Last_SQL_Errno:" | awk -F':' '{print $2}' | tr -d '[[:space:]]')
        LASTSQLERR=$(echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep "Last_SQL_Error:" | sed 's/Last_SQL_Error://g' | sed 's/^[[:space:]]*//' )
        case "$LASTSQLERRNO" in
                1032|1062)
                                ...
                                echo "$SKIPCOUNTQRY" | mysql --login-path=${LOGINPATH}
                                echo "[`date +"%Y-%m-%d %H:%M:%S"`]${cyn}[SLEEPING]Sleeping ${CHECKSLEEPTIME}s before new check${off}"
                                sleep ${CHECKSLEEPTIME}s
                                SECSBEHIND=$(echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep Seconds_Behind_Master | tr -d '[[:space:]]' | awk -F':' '{print $2}')
                            #echo "SKIPCOUNTQRY" | mysql --login-path=${LOGINPATH}
                        ;;
                0)
                ...
                *)
                 ...
                 SECSBEHIND=$(echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep Seconds_Behind_Master | tr -d '[[:space:]]' | awk -F':' '{print $2}')
                 echo "SHOW SLAVE STATUS\G" | mysql --login-path=${LOGINPATH} | grep Err | sed 's/^[[:space:]]*//'
