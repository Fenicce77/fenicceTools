...
# 1. Get Row Count
TOTAL_ROWS=$(mysql --login-path="$LOGIN_PATH" -N -s -e "SELECT TABLE_ROWS FROM information_schema.tables WHERE table_schema='$DATABASE' AND table_name='$TABLE';")

if [ -z "$TOTAL_ROWS" ] || [ "$TOTAL_ROWS" -eq "0" ]; then
    TOTAL_ROWS=$(mysql --login-path="$LOGIN_PATH" -N -s -e "SELECT COUNT(*) FROM $DATABASE.$TABLE;")
fi

echo "--- Advanced Report: $DATABASE.$TABLE ---"
echo "Estimated Rows: $TOTAL_ROWS"
...
for COL in $COLUMNS; do
    # Get Index Info: Format is "IndexName(Pos),Index2(Pos)"
    ...
    # 3. Strategy Selection for Cardinality
    if [ "$TOTAL_ROWS" -gt "$THRESHOLD" ]; then
