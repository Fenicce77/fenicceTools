...
# Deduplicación inicial (Compatible con macOS/Bash 3.2 sin usar readarray)
...
for current_file in "${FILES_TO_PROCESS[@]}"; do
    ...
    FNR == 1 { file_start_ts = ""; in_deadlock = 0; in_trans = 0; deadlock_ts = ""; ignore_deadlock = 0; current_ts = ""; }
    ...
    /^20[0-9][0-9]-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9]:[0-9][0-9]/ {
        ...
        } else if ($0 ~ /INNODB MONITOR OUTPUT/ || current_ts == "") { 
            # Blindaje para evitar arrastrar fechas viejas de errores residuales en el log
            current_ts = $1 " " $2; 
