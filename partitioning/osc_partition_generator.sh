...
NUM_YEARS=20
EXTRA_TEXT=''
OSC_ALTER_OPTION='--ask-pass --recursion-method=none --no-check-alter --nocheck-plan --alter-foreign-keys-method=drop_swap'
OSC_LOAD_OPTION='--max-lag 300 --max-load Threads_running=100,Threads_connected=110 --critical-load Threads_connected=150'
DB_PORT=3306
...
# echo -e "\nEnter the target hostname name, followed by [ENTER]:"
# read HOST_NAME;
# if [[ -z ${HOST_NAME} ]]; then echo "Hostname name cannot be empty"; exit 1; fi

#CMD="cat /etc/my.cnf | grep socket | head -1 | cut -d '=' -f2 | tr -d '[:space:]'"
#MYSQl_SOCKET=$(ssh -o StrictHostKeyChecking=no ${USER}@${HOST_NAME} "${CMD}")
#echo "Found MySQL socket: ${MYSQl_SOCKET}"
...
case $(echo $TIME_RANGE | tr '[:upper:]' '[:lower:]') in
	'd'|'day')
		...
		do
			...
			if [[ ${PARTITIONING} == 'COLUMNS' ]]; then
			 	printf "PARTITION p%d VALUES LESS than ('$curday') \n" $s
			 else
	...
	'y'|'year' )
		...
		echo -e ")\" \n ${OSC_LOAD_OPTION} D=${SCHEMA_NAME},t=${TABLE_NAME},S=${MYSQl_SOCKET}"
	;;
	*) echo "[`date +"%Y-%m-%d %H:%M:%S"`]${red}[ERROR] Time range not recognized${off}"; exit 1;;
esac
