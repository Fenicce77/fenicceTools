...
START_DATE=""
END_DATE=""
REPORT_MODE="file" # screen, file, both (Cambiado a 'file' por defecto)

# --- HELP FUNCTION ---
show_help() {
    echo -e "${BOLD}Usage: $0 [OPTIONS]${NC}"
    echo "Analyzes SHOW ENGINE INNODB STATUS logs to extract Deadlocks, persistent Locks, Users, IPs, and Thread IDs."
    echo "Files MUST strictly follow the naming convention: YYYYMMDD_HH.sample"
    echo ""
    echo -e "${BOLD}Options:${NC}"
    echo "  -d, --dir <directory>      Processes all files within the specified directory."
    echo "  -f, --file <file...>       Processes one or multiple explicitly provided files."
    echo "  -p, --pattern <pattern>    Processes files using a wildcard pattern."
    echo "  -s, --start <date>         Process files from this date. Format: \"YYYY-MM-DD [HH[:MM[:SS]]]\""
    echo "                             If no --end is provided, it processes up to the end of this specific day."
    echo "  -e, --end <date>           Process files up to this date. Format: \"YYYY-MM-DD [HH[:MM[:SS]]]\""
    echo "                             If no --start is provided, it processes from the beginning of this specific day."
    echo "  -n, --top <number>         Specifies the number of queries to show in the global summary (Default: 20)."
    echo "  -t, --table <name>         Filters the analysis to show only queries containing this table name."
    echo "  -u, --user <name>          Filters the analysis to show only locks/deadlocks caused by this user."
    echo "  -m, --mode <mode>          Selects the analysis scope: 'all', 'deadlocks', 'locks'."
    echo "  -r, --report-mode <mode>   Detail output destination: 'screen', 'file', or 'both'. (Default: file)"
    echo "  -o, --output-dir <dir>     Directory to save generated CSV reports and logs. If unset, saves to current dir."
...
# Aviso exclusivo por pantalla para notificar al usuario
...
# Always outputs to screen. Outputs to file if REPORT_MODE is 'file' or 'both'
out_always() {
...
# Output respects REPORT_MODE settings explicitly
out_detail() {
    local tmp_out=$(mktemp)
    cat > "$tmp_out"
...
if [[ -n "$OUTPUT_DIR" && "$HAS_DATE_FILTER" == true ]]; then
    ...
    echo "Type,Hash,Timestamp,User,Host,ThreadID,QueryTemplate" > "$GLOBAL_DETAILS_CSV"
fi
...
generate_report_section() {
    ...
    if [[ "$is_deadlock" == "true" ]]; then
        awk -F'|' '
        {
            ts=$2; user=$3; host=$4; thread=$5; query=$6; count[query]++;
        ...
        END { for (q in count) printf "%d\t%s\t%s\t%s\t%s\t%s\n", count[q], intervals[q], users[q], hosts[q], threads[q], q; }
        ' "$file" | sort -nr > "${file}_agg"
        
        while IFS=$'\t' read -r freq timestamps users_list hosts_list threads_list query; do
        ...
        END {
            for(q in count) {
                ...
                printf "%d\t%s\t%s\t%s\t%s\t%s\n", count[q], ranges[q], users[q], hosts[q], threads[q], q;
            }
        }
        ' "$file" | sort -nr > "${file}_agg"
        
        while IFS=$'\t' read -r freq range_data users_list hosts_list threads_list query; do
            hash=$(printf "%s" "$query" | md5sum | cut -c1-12)
...
for current_file in "${FILES_TO_PROCESS[@]}"; do
    ...
    awk -v tbl_pat="$AWK_TABLE_PATTERN" -v usr_pat="$AWK_USER_PATTERN" -v mode="$ANALYSIS_MODE" '
    BEGIN { 
        in_deadlock=0; in_trans=0; is_lock=0; expect_query=0; 
        current_ts=""; file_start_ts=""; deadlock_ts=""; ignore_deadlock=0; 
        current_dl_user="unknown"; current_dl_host="unknown"; current_dl_thread="unknown";
        current_lock_user="unknown"; current_lock_host="unknown"; current_lock_thread="unknown";
    }
    FNR == 1 { file_start_ts = ""; in_deadlock = 0; in_trans = 0; }
    
    /^20[0-9][0-9]-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9]:[0-9][0-9]/ {
        if (!in_deadlock && !in_trans) { current_ts = $1 " " $2; if (file_start_ts == "") file_start_ts = current_ts; }
    }
    /^LATEST DETECTED DEADLOCK/ {
        in_deadlock=1; getline; 
        if ($0 ~ /^20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]/) deadlock_ts = $1 " " $2; else deadlock_ts = "0000-00-00 00:00:00";
        if (deadlock_ts < file_start_ts) ignore_deadlock = 1; else ignore_deadlock = 0;
        next;
    }
    ...
    /MySQL thread id/ {
        ...
        if (in_deadlock) { current_dl_user = user; current_dl_host = host; current_dl_thread = thread; }
    ...
    expect_query {
        ...
        if (query ~ /^[Ii][Nn][Ss][Ee][Rr][Tt]|[Uu][Pp][Dd][Aa][Tt][Ee]|[Dd][Ee][Ll][Ee][Tt][Ee]|[Ss][Ee][Ll][Ee][Cc][Tt]|[Rr][Ee][Pp][Ll][Aa][Cc][Ee]/) {
            ...
            template = query; 
            gsub(/[0-9]+/, "?", template); gsub(/\047[^\047]*\047/, "?", template); gsub(/"[^"]*"/, "?", template);
            
            if (in_deadlock && !ignore_deadlock && mode != "locks") {
                print "DEADLOCK|" deadlock_ts "|" current_dl_user "|" current_dl_host "|" current_dl_thread "|" template;
            } else if (is_lock && mode != "deadlocks") { 
                print "LOCK|" current_ts "|" current_lock_user "|" current_lock_host "|" current_lock_thread "|" template; 
            }
        }
        is_lock = 0; 
    }
    ...
    if [[ -n "$OUTPUT_DIR" ]]; then
        if [[ "$HAS_DATE_FILTER" == true ]]; then
            while IFS='|' read -r type ts user host thread q; do
                hash=$(printf "%s" "$q" | md5sum | cut -c1-12)
                printf "%s,%s,%s,%s,%s,%s,\"%s\"\n" "$type" "$hash" "$ts" "$user" "$host" "$thread" "$q"
            done < "$TMP_RAW" >> "$GLOBAL_DETAILS_CSV"
        else
            OUTPUT_CSV="${OUTPUT_DIR}/${FILE_NAME_BASE}.analysis_${ANALYSIS_MODE^^}_${PARAM_STR}.csv"
            echo "Type,Hash,Timestamp,User,Host,ThreadID,QueryTemplate" > "$OUTPUT_CSV"
            while IFS='|' read -r type ts user host thread q; do
                hash=$(printf "%s" "$q" | md5sum | cut -c1-12)
                printf "%s,%s,%s,%s,%s,%s,\"%s\"\n" "$type" "$hash" "$ts" "$user" "$host" "$thread" "$q"
            done < "$TMP_RAW" >> "$OUTPUT_CSV"
...
if [[ ${#FILES_TO_PROCESS[@]} -gt 1 || "$HAS_DATE_FILTER" == true ]]; then
    ...
    {
        type=$1; ts=$2; user=$3; host=$4; thread=$5; query=$6;
    ...
    END {
        ...
        for (q in occurrences) {
            ...
            printf "DATA|%d|%d|%d|%d|%s|%s|%s|%s|%s\n", score, td, dl, occ, users[q], t_str, first_seen[q], last_seen[q], q;
        }
    ...
    # BEGIN ALWAYS OUTPUT BLOCK (Summary Table goes to Screen and Log File)
    {
        ...
        if [[ "$SHOW_DEADLOCK_COLUMN" == true ]]; then
            printf "${BOLD}%-12s | %-19s | %-19s | %-13s | %-9s | %-11s | %-35s | %-55s | %s${NC}\n" "HASH" "FIRST_SEEN" "LAST_SEEN" "LockTime" "Deadlocks" "Occurrences" "USERS" "THREADS" "QUERY TEMPLATE"
            echo "-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
        else
            printf "${BOLD}%-12s | %-19s | %-19s | %-13s | %-11s | %-35s | %-55s | %s${NC}\n" "HASH" "FIRST_SEEN" "LAST_SEEN" "LockTime" "Occurrences" "USERS" "THREADS" "QUERY TEMPLATE"
            echo "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
        fi

        grep "^DATA|" "${GLOBAL_UNIQUE_RAW}_agg" | sort -t'|' -k2 -nr | head -n "$TOP_LIMIT" | while IFS='|' read -r type score t_lock deadlocks occ users_list threads_list first_seen last_seen q; do
            hash=$(printf "%s" "$q" | md5sum | cut -c1-12)
            ...
            short_users=$(echo "$users_list" | cut -c1-32); [[ ${#users_list} -gt 32 ]] && short_users+="..."
            visual_threads=$(echo "$threads_list" | cut -c1-52); [[ ${#threads_list} -gt 52 ]] && visual_threads+="..."

            if [[ "$SHOW_DEADLOCK_COLUMN" == true ]]; then
                printf "${row_color}%-12s | %-19.19s | %-19.19s | %-13.13s | %-9.9s | %-11.11s | %-35.35s | %-55.55s | %s${NC}\n" "$hash" "$first_seen" "$last_seen" "${t_lock}s" "$deadlocks" "$occ" "$short_users" "$visual_threads" "$short_q"
            else
                printf "${row_color}%-12s | %-19.19s | %-19.19s | %-13.13s | %-11.11s | %-35.35s | %-55.55s | %s${NC}\n" "$hash" "$first_seen" "$last_seen" "${t_lock}s" "$occ" "$short_users" "$visual_threads" "$short_q"
            fi
        done
    ...
    # Exportación Full Recap (Sin límite TOP y ajeno al flujo de pantalla)
    if [[ -n "$OUTPUT_DIR" ]]; then
        RECAP_CSV="${OUTPUT_DIR}/full_recap_${ANALYSIS_MODE^^}_${DATE_STR_FOR_FILE}${PARAM_STR}.csv"
        echo "Hash,FirstSeen,LastSeen,LockTime,Deadlocks,Occurrences,Users,Threads,QueryTemplate" > "$RECAP_CSV"
        
        grep "^DATA|" "${GLOBAL_UNIQUE_RAW}_agg" | sort -t'|' -k2 -nr | while IFS='|' read -r _ score t_lock dl occ users threads f_seen l_seen q; do
            hash=$(printf "%s" "$q" | md5sum | cut -c1-12)
            printf "%s,%s,%s,%ds,%d,%d,\"%s\",\"%s\",\"%s\"\n" "$hash" "$f_seen" "$l_seen" "$t_lock" "$dl" "$occ" "$users" "$threads" "$q"
        done >> "$RECAP_CSV"
