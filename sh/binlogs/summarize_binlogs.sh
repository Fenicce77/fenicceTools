...
# ./summarize_binlogs.sh $1 $2 $3 | grep Table |cut -d':' -f5| cut -d' ' -f2 | sort | uniq -c | sort -nr
# 2.- Check highest delete operation tables
# ./summarize_binlogs.sh $1 $2 $3 | grep -E 'DELETE' |cut -d':' -f5| cut -d' ' -f2 | sort | uniq -c | sort -nr
# 3.- Check operations executed on a specific table
# ./summarize_binlogs.sh $1 $2 $3 | grep -i '`<schema>`.`<table>`' | awk '{print $7 " " $11}' | sort -k1,2 | uniq -c

# Examples :
# 
# summarize_binlogs.sh /mysql/mysqllog/bin-log.002576 "2021-07-13 15:00:00" "2021-07-13 15:10:00" | grep Table |cut -d':' -f5| cut -d' ' -f2 | sort | uniq -c | sort -nr
# summarize_binlogs.sh /mysql/mysqllog/bin-log.002576 "2021-07-13 15:00:00" "2021-07-13 15:10:00" | grep -E 'DELETE' |cut -d':' -f5| cut -d' ' -f2 | sort | uniq -c | sort -nr
# summarize_binlogs.sh /mysql/mysqllog/bin-log.002576 "2021-07-13 15:00:00" "2021-07-13 15:10:00" | grep -i '`<schema>`.`<table>`' | awk '{print $7 " " $11}' | sort -k1,2 | uniq -c
