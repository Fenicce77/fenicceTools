...
do
	...
	echo "SELECT USER,DB,substring_index(substring_index(HOST,':',1),'.',4) as HH,count(*) as total FROM information_schema.PROCESSLIST group by USER,DB,HH ORDER BY user,total;" | $MYSQLBIN --defaults-file=${MYCONF} -t -A information_schema
