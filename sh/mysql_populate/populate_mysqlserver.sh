...
INITROWSINTABLE=`echo "select count(*) as total from t1;"|mysql --defaults-file=${MYCONFILE} -A jucy`
echo "[`date +"%Y-%m-%d %H:%M:%S"]` Initial Rows in table : ${INITROWSINTABLE}"
for i in {1..500};do
	host=`hostname`
	qrynew="INSERT INTO t1 (K,host,hostfrom,created_ts) VALUES (${i},@@hostname,'${HOSTFROM}',now());"
...
fulltrans="${qryBegin}${qrytrans}${qryCommit}"
#echo $fulltrans
echo $fulltrans | mysql --defaults-file=${MYCONFILE} -A jucy
ROWSINTABLE=`echo "select count(*) as total from t1;"|mysql --defaults-file=${MYCONFILE} -A jucy`
