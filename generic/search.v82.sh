...
show_help() {
    echo -e "${cyn}==============================================================================================================${off}"
    echo -e "${bld}Hexon DB Search Tool v8.2 - Omnipresent Search in MySQL instances${off}"
    echo -e "${cyn}==============================================================================================================${off}"
    ...
    echo -e "  ${grn}-d, --detailed${off}    Extracts advanced info (Cloud Privileges, Hierarchical Roles, Limits, Collations, etc.)."
...
if [[ $RELATED -eq 1 ]]; then
    if [[ $EXACT -eq 0 ]]; then
        ...
        # Inyectamos búsquedas secundarias si se está buscando una tabla ('t') o todo ('all')
    ...
    for act in $ACTION_LIST; do
        case $act in
            ...
            'p'|'procedure'|'proc')
                OBJNAME='Routine'
                if [[ $DETAILED -eq 1 ]]; then
                    QUERY="SELECT @@hostname, '$RAW_VERSION', ROUTINE_SCHEMA, ROUTINE_NAME, ROUTINE_TYPE, CONCAT('Definer: ', DEFINER, ' | Conn_Coll: ', IFNULL(COLLATION_CONNECTION,'N/A'), ' | DB_Coll: ', IFNULL(DATABASE_COLLATION,'N/A'), ' | Created: ', CREATED, ' | Modified: ', LAST_ALTERED) FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME $SEARCH_CLAUSE AND ROUTINE_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                else
            ...
            'p_rel')
                # MODO ESPECIAL DE DEPENDENCIAS: Rutinas actuando sobre las tablas buscadas
                ...
                if [[ $DETAILED -eq 1 ]]; then
                    QUERY="SELECT @@hostname, '$RAW_VERSION', ROUTINE_SCHEMA, ROUTINE_NAME, ROUTINE_TYPE, CONCAT('Acts on: [', '$CLEAN_SEARCH', '] | Definer: ', DEFINER, ' | Conn_Coll: ', IFNULL(COLLATION_CONNECTION,'N/A'), ' | DB_Coll: ', IFNULL(DATABASE_COLLATION,'N/A'), ' | Created: ', CREATED, ' | Modified: ', LAST_ALTERED) FROM INFORMATION_SCHEMA.ROUTINES WHERE ($ROUTINE_LIKE_CLAUSE) AND ROUTINE_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                else
                    QUERY="SELECT @@hostname, '$RAW_VERSION', ROUTINE_SCHEMA, ROUTINE_NAME, ROUTINE_TYPE, CONCAT('Acts on: [', '$CLEAN_SEARCH', '] | Definer: ', DEFINER, ' | Created: ', CREATED, ' | Modified: ', LAST_ALTERED) FROM INFORMATION_SCHEMA.ROUTINES WHERE ($ROUTINE_LIKE_CLAUSE) AND ROUTINE_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                fi
                ;;
            ...
            'tr'|'trigger')
                ...
                if [[ $DETAILED -eq 1 ]]; then
                    QUERY="SELECT @@hostname, '$RAW_VERSION', TRIGGER_SCHEMA, TRIGGER_NAME, 'TRIGGER', CONCAT('Definer: ', DEFINER, ' | Conn_Coll: ', IFNULL(COLLATION_CONNECTION,'N/A'), ' | DB_Coll: ', IFNULL(DATABASE_COLLATION,'N/A'), ' | Created: ', $IF_CREATED) FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_NAME $SEARCH_CLAUSE AND TRIGGER_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                else
                    QUERY="SELECT @@hostname, '$RAW_VERSION', TRIGGER_SCHEMA, TRIGGER_NAME, 'TRIGGER', CONCAT('Definer: ', DEFINER, ' | Created: ', $IF_CREATED) FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_NAME $SEARCH_CLAUSE AND TRIGGER_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                fi
                ;;

            'tr_rel')
                # MODO ESPECIAL DE DEPENDENCIAS: Triggers acoplados a las tablas buscadas
                OBJNAME='Trigger (Dependency)'
                IF_CREATED=$([[ "$MAJOR_VER" -ge 8 ]] && echo "IFNULL(CREATED,'N/A')" || echo "'N/A (v5.7)'")
                if [[ $DETAILED -eq 1 ]]; then
                    QUERY="SELECT @@hostname, '$RAW_VERSION', TRIGGER_SCHEMA, TRIGGER_NAME, 'TRIGGER', CONCAT('Bound Table: ', EVENT_OBJECT_TABLE, ' | Definer: ', DEFINER, ' | Conn_Coll: ', IFNULL(COLLATION_CONNECTION,'N/A'), ' | DB_Coll: ', IFNULL(DATABASE_COLLATION,'N/A'), ' | Created: ', $IF_CREATED) FROM INFORMATION_SCHEMA.TRIGGERS WHERE EVENT_OBJECT_TABLE $SEARCH_CLAUSE AND TRIGGER_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                else
                    QUERY="SELECT @@hostname, '$RAW_VERSION', TRIGGER_SCHEMA, TRIGGER_NAME, 'TRIGGER', CONCAT('Bound Table: ', EVENT_OBJECT_TABLE, ' | Definer: ', DEFINER, ' | Created: ', $IF_CREATED) FROM INFORMATION_SCHEMA.TRIGGERS WHERE EVENT_OBJECT_TABLE $SEARCH_CLAUSE AND TRIGGER_SCHEMA NOT IN ($IGNORE_SCHEMAS);"
                fi
                ;;
            ...
            'u'|'user')
                OBJNAME='User'
                if [[ $DETAILED -eq 1 ]]; then
                    GRANTEE_STR="CONCAT(CHAR(39), user, CHAR(39), '@', CHAR(39), host, CHAR(39))"
                    
                    SQL_GLOBAL_PRIVS="(SELECT IFNULL((SELECT IF(COUNT(PRIVILEGE_TYPE)>20, IF(SUM(IS_GRANTABLE='YES')>0, 'ALL GRANTS WITH GRANT OPTION', 'ALL GRANTS'), GROUP_CONCAT(PRIVILEGE_TYPE)) FROM information_schema.user_privileges WHERE GRANTEE = TARGET_GRANTEE AND PRIVILEGE_TYPE != 'USAGE'), 'None'))"
                    SQL_SCHEMA_PRIVS="(SELECT IFNULL((SELECT GROUP_CONCAT(CONCAT(TABLE_SCHEMA, ' [', privs, ']') SEPARATOR ' • ') FROM (SELECT GRANTEE, TABLE_SCHEMA, IF(COUNT(PRIVILEGE_TYPE)>14, IF(SUM(IS_GRANTABLE='YES')>0, 'ALL GRANTS WITH GRANT OPTION', 'ALL GRANTS'), GROUP_CONCAT(PRIVILEGE_TYPE)) as privs FROM information_schema.schema_privileges GROUP BY GRANTEE, TABLE_SCHEMA) sp WHERE sp.GRANTEE = TARGET_GRANTEE), 'None'))"
                    ...
                    if [[ "$MAJOR_VER" -ge 8 ]]; then
                        ROLES_STR="IFNULL((SELECT GROUP_CONCAT(CONCAT(FROM_USER, '@', FROM_HOST)) FROM mysql.role_edges WHERE TO_USER = user AND TO_HOST = host), 'None')"
                        
                        ROLE_GRANTEE_STR="CONCAT(CHAR(39), FROM_USER, CHAR(39), '@', CHAR(39), FROM_HOST, CHAR(39))"
                        ROLE_GLOBAL="${SQL_GLOBAL_PRIVS//TARGET_GRANTEE/$ROLE_GRANTEE_STR}"
                        ROLE_DB="${SQL_SCHEMA_PRIVS//TARGET_GRANTEE/$ROLE_GRANTEE_STR}"
                        
                        ROLE_DETAILS_STR="IFNULL((SELECT GROUP_CONCAT(CONCAT('__ROLE_BR__', FROM_USER, '@', FROM_HOST, '__ROLE_DIV__Global: ', $ROLE_GLOBAL, ' | DB: ', $ROLE_DB) SEPARATOR '') FROM mysql.role_edges WHERE TO_USER = user AND TO_HOST = host), '')"
                        
                        QUERY="SELECT @@hostname, '$RAW_VERSION', user, host, 'USER', CONCAT('Auth: ', plugin, ' | Roles: ', $ROLES_STR, ' | Direct Global: ', $DIRECT_GLOBAL, ' | Direct DB: ', $DIRECT_DB, $LIMITS_STR, $ROLE_DETAILS_STR, '__ROLE_END__') FROM mysql.user WHERE user $SEARCH_CLAUSE;"
                    else
                        QUERY="SELECT @@hostname, '$RAW_VERSION', user, host, 'USER', CONCAT('Auth: ', plugin, ' | Direct Global: ', $DIRECT_GLOBAL, ' | Direct DB: ', $DIRECT_DB, $LIMITS_STR) FROM mysql.user WHERE user $SEARCH_CLAUSE;"
                    fi
                else
                    QUERY="SELECT @@hostname, '$RAW_VERSION', user, host, 'USER', CONCAT('Auth: ', plugin) FROM mysql.user WHERE user $SEARCH_CLAUSE;"
                fi
                ;;
