...
show_help() {
    echo -e "${cyn}==============================================================================================================${off}"
    echo -e "${bld}Hexon DB Search Tool v7.5 - Omnipresent Search in MySQL instances${off}"
    echo -e "${cyn}==============================================================================================================${off}"
    ...
    echo -e "  ${grn}-d, --detailed${off}    Extracts advanced info (Table sizes/updates, Index sizes, Collations, etc.)."
...
for s in $SERVERS_TO_CHECK; do
    ...
    for act in $ACTION_LIST; do
        case $act in
            ...
            'v'|'view')
                OBJNAME='View'
                # AÑADIDA FECHA DE ACTUALIZACIÓN A VISTAS EN DETALLADO
                if [[ $DETAILED -eq 1 ]]; then
            ...
            'p'|'procedure'|'proc')
                OBJNAME='Routine'
                # CORREGIDO: LAS FECHAS YA NO SE SOBREESCRIBEN EN MODO DETALLADO
                if [[ $DETAILED -eq 1 ]]; then
            ...
            'tr'|'trigger')
                ...
                # CORREGIDO: LA FECHA DE CREACIÓN YA NO SE PIERDE EN MODO DETALLADO
                if [[ $DETAILED -eq 1 ]]; then
            ...
            'e'|'event')
                OBJNAME='Event'
                # LOS EVENTOS SIEMPRE HAN MOSTRADO CREATED Y LAST_ALTERED
        ...
        if [[ -n "$RESULT" ]]; then
            ...
            echo "$RESULT" | column -t -s $'\t' | sed "s|__NO_CHARSET__|${red}-${off}|g" | sed 's/^/      /'
            echo ""
            
            [[ $CSV_EXPORT -eq 1 ]] && echo "$RESULT" | sed 's/__NO_CHARSET__/-/g' | sed 's/\t/|/g' >> "$CSV_FILE"
        fi
    done
