...
show_help() {
    echo -e "${cyn}==============================================================================================================${off}"
    echo -e "${bld}Hexon DB Search Tool v6.5 - Multi-Object Search in MySQL instances${off}"
    echo -e "${cyn}==============================================================================================================${off}"
...
# ==========================================
case $ACTION in
    's'|'schema')    OBJNAME='Schema' ;;
    't'|'table')     OBJNAME='Table' ;;
    'v'|'view')      OBJNAME='View' ;;
    'p'|'procedure') OBJNAME='Routine' ;;
    'tr'|'trigger')  OBJNAME='Trigger' ;;
    'e'|'event')     OBJNAME='Event' ;;
    'u'|'user')      OBJNAME='User' ;;
    'c'|'column')    OBJNAME='Column' ;;
    'i'|'index')     OBJNAME='Index' ;;
    *) 
        echo -e "${red}[ERROR] Invalid object type: '${ACTION}'. Please check the help menu.${off}\n"
        show_help
        exit 1
        ;;
esac
...
# Modificamos el mensaje para usar la variable bonita $OBJNAME
if [[ $INCLUDE_SYS -eq 1 ]]; then
    echo -e "${cyn}Starting multi-object search (INCLUDING SYS SCHEMAS) for ${OBJNAME}(s) -> [${TOSEARCH}]...${off}\n"
else
    echo -e "${cyn}Starting multi-object search for ${OBJNAME}(s) -> [${TOSEARCH}]...${off}\n"
fi
...
for s in $SERVERS_TO_CHECK; do
    ...
    # El bloque case ahora solo se encarga de crear la QUERY correspondiente
    case $ACTION in
        's'|'schema')
    ...
    if [[ -n "$RESULT" ]]; then
        FOUND_SERVERS+=("$s")
        echo -e "${grn}[✓] $OBJNAME Found in:${off} ${mag}${s}${off} ${wht}(v${RAW_VERSION})${off}"
